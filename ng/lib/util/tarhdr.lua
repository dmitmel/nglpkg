...
function ng.tarhdr.convertField(oct)
	local v = 0
	for i = 1, #oct do
...
function ng.tarhdr.getInfo(sector)
	...
	info.access = ng.tarhdr.convertField(sector:sub(101, 107))
	info.uid = ng.tarhdr.convertField(sector:sub(109, 115))
	info.gid = ng.tarhdr.convertField(sector:sub(117, 123))
	info.size = ng.tarhdr.convertField(sector:sub(125, 135))
